/*--------------------------------------------------------------------------------------*/
/*																						*/
/*				WiringPi RADXA ZERO 2 Board control file								*/
/*																						*/
/*--------------------------------------------------------------------------------------*/
...
void init_radxa_zero2(struct libkhadas *libwiring);
...
static int gpioToGPSETReg (int pin)
{
    if(pin >= ZERO2_GPIOA_PIN_START && pin <= ZERO2_GPIOA_PIN_END)
		return ZERO2_GPIOA_OUTP_REG_OFFSET;
	if(pin >= ZERO2_GPIOH_PIN_START && pin <= ZERO2_GPIOH_PIN_END)
		return ZERO2_GPIOH_OUTP_REG_OFFSET;
	if(pin >= ZERO2_GPIOAO_PIN_START && pin <= ZERO2_GPIOAO_PIN_END)
		return ZERO2_GPIOAO_OUTP_REG_OFFSET;
	if(pin >= ZERO2_GPIOZ_PIN_START && pin <= ZERO2_GPIOZ_PIN_END)
		return ZERO2_GPIOZ_OUTP_REG_OFFSET;
	return -1; 
}
...
{
	if(pin >= ZERO2_GPIOA_PIN_START && pin <= ZERO2_GPIOA_PIN_END)
		return ZERO2_GPIOA_INP_REG_OFFSET;
	if(pin >= ZERO2_GPIOH_PIN_START && pin <= ZERO2_GPIOH_PIN_END)
		return ZERO2_GPIOH_INP_REG_OFFSET;
	if(pin >= ZERO2_GPIOAO_PIN_START && pin <= ZERO2_GPIOAO_PIN_END)
		return ZERO2_GPIOAO_INP_REG_OFFSET;
	if(pin >= ZERO2_GPIOZ_PIN_START && pin <= ZERO2_GPIOZ_PIN_END)
		return ZERO2_GPIOZ_INP_REG_OFFSET;
	return -1; 
}
...
{
    if(pin >= ZERO2_GPIOA_PIN_START && pin <= ZERO2_GPIOA_PIN_END)
		return ZERO2_GPIOA_PUEN_REG_OFFSET;
	if(pin >= ZERO2_GPIOH_PIN_START && pin <= ZERO2_GPIOH_PIN_END)
		return ZERO2_GPIOH_PUEN_REG_OFFSET;
	if(pin >= ZERO2_GPIOZ_PIN_START && pin <= ZERO2_GPIOZ_PIN_END)
		return ZERO2_GPIOZ_PUEN_REG_OFFSET;
	if(pin >= ZERO2_GPIOAO_PIN_START && pin <= ZERO2_GPIOAO_PIN_END)
		return ZERO2_GPIOAO_PUEN_REG_OFFSET;
	return -1;
}
...
{
    if(pin >= ZERO2_GPIOA_PIN_START && pin <= ZERO2_GPIOA_PIN_END)
		return ZERO2_GPIOA_PUPD_REG_OFFSET;
	if(pin >= ZERO2_GPIOH_PIN_START && pin <= ZERO2_GPIOH_PIN_END)
		return ZERO2_GPIOH_PUPD_REG_OFFSET;
	if(pin >= ZERO2_GPIOZ_PIN_START && pin <= ZERO2_GPIOZ_PIN_END)
		return ZERO2_GPIOZ_PUPD_REG_OFFSET;
	if(pin >= ZERO2_GPIOAO_PIN_START && pin <= ZERO2_GPIOAO_PIN_END)
		return ZERO2_GPIOAO_PUPD_REG_OFFSET;
	return -1;
}
...
{
    if(pin >= ZERO2_GPIOA_PIN_START && pin <= ZERO2_GPIOA_PIN_END)
		return pin - ZERO2_GPIOA_PIN_START;
	if(pin >= ZERO2_GPIOH_PIN_START && pin <= ZERO2_GPIOH_PIN_END)
		return pin - ZERO2_GPIOH_PIN_START;
	if(pin >= ZERO2_GPIOZ_PIN_START && pin <= ZERO2_GPIOZ_PIN_END)
		return pin - ZERO2_GPIOZ_PIN_START;
	if(pin >= ZERO2_GPIOAO_PIN_START && pin <= ZERO2_GPIOAO_PIN_END)
		return pin - ZERO2_GPIOAO_PIN_START;
	return -1;
}
...
{
    if(pin >= ZERO2_GPIOA_PIN_START && pin <= ZERO2_GPIOA_PIN_END)
	    return ZERO2_GPIOA_FSEL_REG_OFFSET;
	if(pin >= ZERO2_GPIOH_PIN_START && pin <= ZERO2_GPIOH_PIN_END)
		return ZERO2_GPIOH_FSEL_REG_OFFSET;
	if(pin >= ZERO2_GPIOZ_PIN_START && pin <= ZERO2_GPIOZ_PIN_END)
		return ZERO2_GPIOZ_FSEL_REG_OFFSET;
	if(pin >= ZERO2_GPIOAO_PIN_START && pin <= ZERO2_GPIOAO_PIN_END)
		return ZERO2_GPIOAO_FSEL_REG_OFFSET;
	return -1;
}
...
{
    if(pin >= ZERO2_GPIOA_PIN_START && pin <= ZERO2_GPIOA_PIN_END)
	    return ZERO2_GPIOA_DS_REG_5A_OFFSET;
	if(pin >= ZERO2_GPIOH_PIN_START && pin <= ZERO2_GPIOH_PIN_END)
		return ZERO2_GPIOH_DS_REG_3A_OFFSET;
	if(pin >= ZERO2_GPIOZ_PIN_START && pin <= ZERO2_GPIOZ_PIN_END)
		return ZERO2_GPIOZ_DS_REG_4_OFFSET;
	if(pin >= ZERO2_GPIOAO_PIN_START && pin <= ZERO2_GPIOAO_PIN_END)
		return ZERO2_GPIOAO_DS_REG_A_OFFSET;
	return -1;
}
...
{	
	switch(pin){
		case ZERO2_GPIOA_PIN_START  ...ZERO2_GPIOA_PIN_START + 7:
			return  ZERO2_GPIOA_MUX_D_REG_OFFSET;
		case ZERO2_GPIOZ_PIN_START  ...ZERO2_GPIOZ_PIN_START + 7:
			return  ZERO2_GPIOZ_MUX_B_REG_OFFSET;
		case ZERO2_GPIOA_PIN_START + 8  ...ZERO2_GPIOA_PIN_END:
			return  ZERO2_GPIOA_MUX_E_REG_OFFSET;
		case ZERO2_GPIOZ_PIN_START + 8  ...ZERO2_GPIOZ_PIN_END:
			return  ZERO2_GPIOZ_MUX_C_REG_OFFSET;
		case ZERO2_GPIOH_PIN_START  ...ZERO2_GPIOH_PIN_END:
			return ZERO2_GPIOH_MUX_B_REG_OFFSET;
		case ZERO2_GPIOAO_PIN_START ...ZERO2_GPIOAO_PIN_START + 7:
			return ZERO2_GPIOAO_MUX_0_REG_OFFSET;
		case ZERO2_GPIOAO_PIN_START + 8 ...ZERO2_GPIOAO_PIN_END:
			return ZERO2_GPIOAO_MUX_1_REG_OFFSET;
	}
	return ZERO2_GPIOAO_MUX_1_REG_OFFSET;
}
...
{       
	...
	divisor = (divisor - 1);
		...
		*( pwm + ZERO2_PWM_MISC_REG_01_OFFSET ) = \
			(1 << ZERO2_PWM_1_CLK_EN) \
			| ( divisor << ZERO2_PWM_1_CLK_DIV0) \
			| (1 << ZERO2_PWM_0_CLK_EN) \
			| ( divisor << ZERO2_PWM_0_CLK_DIV0) \ 
			| (0 << ZERO2_PWM_1_CLK_SEL0) \
			| (0 << ZERO2_PWM_0_CLK_SEL0) \
			| (1 << ZERO2_PWM_1_EN) \
			| (1 << ZERO2_PWM_0_EN);
}
...
static int _pwmWrite (int pin, int value)
{
    ...
    *(pwm + ZERO2_PWM_1_DUTY_CYCLE_OFFSET) = (value << 16) | (range - value);
    *(pwm + ZERO2_PWM_0_DUTY_CYCLE_OFFSET) = (value << 16) | (range - value);
...
static int init_gpio_mmap(void)
{
	...
	gpio1  = (uint32_t *)mmap(0, BLOCK_SIZE, PROT_READ|PROT_WRITE,
						MAP_SHARED, fd1, ZERO2_GPIOAO_BASE);
	gpio  = (uint32_t *)mmap(0, BLOCK_SIZE, PROT_READ|PROT_WRITE,
						MAP_SHARED, fd, ZERO2_GPIO_BASE);
	...
	pwm = ( uint32_t * )mmap( 0, BLOCK_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, ZERO2_GPIO_PWM_BASE);
...
void init_radxa_zero2(struct libkhadas *libwiring)
{
	...
	libwiring->pinBase = ZERO2_GPIO_PIN_BASE;
